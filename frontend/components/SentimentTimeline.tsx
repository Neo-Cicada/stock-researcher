...
export default function SentimentTimeline({
  ...
  return (
    <svg viewBox="0 0 760 64" style={{ width: "100%", height: "auto", display: "block" }}>
      ...
      {/* CSS tooltips can't hang off SVG text — <title> gives the same hover gloss. */}
      <text x={718} y={14} fontFamily="IBM Plex Mono, monospace" fontSize={9.5} fill="#4A7C59" style={{ cursor: "help" }}>
        <title>{glossFor("強気")}</title>
        強気
      </text>
      <text x={718} y={58} fontFamily="IBM Plex Mono, monospace" fontSize={9.5} fill="#C3423F" style={{ cursor: "help" }}>
        <title>{glossFor("弱気")}</title>
        弱気
      </text>
