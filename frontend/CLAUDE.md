...
- `/events` — Economic-events calendar (CPI/jobs/GDP/PCE/PPI release dates + FOMC) from FRED as a paginated, date-grouped almanac (`EventsBoard`) with impact discs (high/medium/low); mock fallback (FRED provides real dates/names/impact but no estimate values)
